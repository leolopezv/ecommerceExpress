...
router.get('/findAllByDescripcion/json', function(req, res, next) {
    let order = req.query.order === 'desc' ? 'DESC' : 'ASC'; // Por defecto, es ascendente
    Foto.findAll({
        attributes: { exclude: ["createdAt", "updatedAt"] },
        order: [
            ['descripcion', order] // Ordenar por el atributo "descripcion"
        ]
    })
    .then(fotos => {
        res.json(fotos);
